...
def load_data_to_dwh(conn, name_table, method, load_data=None):
    ...
    if name_table[:3] == 'rep':
        if os.path.getmtime(f'C:/Общая/_DWH/_Reports/{name_table}.csv') > dt_start_day:
            load_data = pd.read_csv(f'C:/Общая/_DWH/_Reports/{name_table}.csv', header=None, skiprows=[0], sep=';', dtype=str)
    elif name_table[:3] == 'dim':
        if os.path.getmtime(f'C:/Общая/_DWH/_Dimension/{name_table}.csv') > (dt_start_day - 86400):
            load_data = pd.read_csv(f'C:/Общая/_DWH/_Dimension/{name_table}.csv', header=None, skiprows=[0], sep=';', dtype=str)
    elif name_table[:3] == 'inf':
        if os.path.getmtime(f'C:/Общая/_DWH/_Info/{name_table}.csv') > dt_start_day:
            load_data = pd.read_csv(f'C:/Общая/_DWH/_Info/{name_table}.csv', header=None, skiprows=[0], sep=';', dtype=str)
        ...
        if os.path.getmtime(f'C:/Общая/_DWH/_Facts_by_periods/SalesPlan/{name_table}.csv') > dt_start_day:
            load_data = pd.read_csv(f'C:/Общая/_DWH/_Facts_by_periods/SalesPlan/{name_table}.csv', header=None, skiprows=[0], sep=';', dtype=str)
